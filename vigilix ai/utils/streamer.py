...
def save_alert(camera, detection, frame):
    ...
    conn.execute(
        """
        INSERT INTO alerts (user_id, camera_id, threat_type, risk_level, confidence, evidence_path, alert_date, alert_day, alert_time, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (camera["user_id"], camera["id"], label, risk, str(round(float(confidence), 2)), filename, alert_date, alert_day, alert_time, created)
    )
    conn.commit()
    conn.close()
...
def error_frame(message):
    ...
    cv2.putText(frame, "Try source 0 for laptop camera", (90, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2)
    return frame
...
def generate_frames(camera, detector):
    ...
    while True:
        ...
        cv2.putText(frame, f"{camera['camera_name']} | {camera['location']} | {camera['location_type']}", (14, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (255,255,255), 2)
        cv2.putText(frame, f"Live Safety Monitoring | Events: {len(latest_detections)}", (14, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0,255,255), 1)
        if alert_present:
            cv2.rectangle(frame, (0, 365), (720, 420), (0, 0, 150), -1)
            cv2.putText(frame, "ALERT DETECTED - EVIDENCE SAVED", (16, 400), cv2.FONT_HERSHEY_SIMPLEX, 0.72, (255,255,255), 2)
